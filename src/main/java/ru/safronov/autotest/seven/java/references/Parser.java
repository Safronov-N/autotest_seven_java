...
public abstract class Parser {
    ...
    private static class StaticParser extends Parser{
        /**
    ...
    static {
        staticParser=new StaticParser();
    }

    public static Parser getInstance(ParserType type){
        switch (type){
            case STATIC: return staticParser;
            case DYNAMIC:
            default:throw new ConfigException("Неизвестный тип парсера");
        }
    }

    protected void initReader() throws XMLStreamException, IOException {
        if (stream!=null)
            stream.reset();
